...
def search_similar(
    ...
    Works across ALL qdrant-client versions by detecting available API.
    ...
    # Method 1: query_points() — works in qdrant-client >= 1.7.0
    if hasattr(qdrant_client, 'query_points'):
        try:
            ...
            last_error = e
            print(f"⚠ query_points failed: {type(e).__name__}: {e}")

    # Method 2: search() — works in qdrant-client < 1.18.0
    ...
    # Method 3: scroll + manual filter — works on ALL versions
    try:
        all_points = []
        offset = None
        while len(all_points) < top_k * 3:  # fetch extra for threshold filtering
            result = qdrant_client.scroll(
                collection_name=collection_name,
                limit=top_k * 3 - len(all_points),
                offset=offset,
                with_payload=True,
                with_vectors=False,
            )
        ...
        # Return all (no vector similarity without embeddings, return raw)
        return [
            {
                "id": str(point.id),
        ...
        print(f"❌ All search methods failed. Last error: {type(e).__name__}: {e}")
        return []
